...
function CountryPage({ params }: { params: Promise<{ code: string }> }) {
	...
	return (
		<Layout>
			<Layout isMain={false}>
				<div className="flex gap-x-4 mb-10 items-center shadow-md rounded-lg bg-darkTextLightElements dark:bg-darkHeader p-1 md:w-48">
				<FontAwesomeIcon
          icon={faArrowLeft}
          className="ml-2 pl-5 w-6 h-6 text-lightText dark:text-darkTextLightElements" 
        />
				<Link
					href="/"
					className="p-2 text-lg text-lightText dark:text-darkTextLightElements"
				>
					Back
				</Link>
				</div>
